...
let convert_test = (snd (convert_grammar awksub_grammar)) Expr
*)
(*
...
*)
let accept_all derivation string = Some (derivation, string)
let accept_plus = fun d -> function 
   | "+"::t -> Some (d,"+"::t)
   | _ -> None
...
let test_1 =
  ((parse_prefix new_grammar accept_all ["1";"+";"3"; "-";"9"]) = 
   Some ([(Expr, [N Term; N Binop; N Expr]); (Term, [N Num]); (Num, [T "1"]);
	 (Binop, [T "+"]); (Expr, [N Term; N Binop; N Expr]); (Term, [N Num]);
	 (Num, [T "3"]); (Binop, [T "-"]); (Expr, [N Term]);
	 (Term, [N Num]);(Num, [T "9"])],[]))

let test_2 =
  ((parse_prefix new_grammar accept_plus ["1";"+";"3"; "-";"9"]) = 
  Some
       ([(Expr, [N Term]); (Term, [N Num]); (Num, [T "1"])],["+";"3";"-";"9"]))
